...
SymbolSection::SymbolSection(const std::string& name, StringSection* namesSection)
        : Section(name), m_namesSection(namesSection), sh_info(0) {
...
void SymbolSection::finalize() {
    ...
    for (const auto& symbol : m_symbols) {
        ...
        symbolEntry.st_shndx =
                symbol->getRelatedSection() ? static_cast<Elf_Half>(symbol->getRelatedSection()->getIndex()) : 0;
