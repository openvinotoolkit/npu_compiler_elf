...
In dKMB UPA SHAVE kernels where statically linked with VPU FW. In NPU3720 due to security reasons the ACT SHAVE kernels must be loaded from Host by UMD. Therefore the kernel images must be included in the blob produced by VPUX compiler. ELF format is better suited for keeping executable code than FlatBuffers format. In particular it natively supports such features as relocation or memory attributes.
